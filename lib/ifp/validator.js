...
function buildInputFromSimbrief(summary, raw, request = {}) {
  return {
    ...
    sid: summary.sidIdent || summary.sid || "",
    sidTransition: summary.sidTrans || summary.sidTransition || "",
  };
}
...
function normalizeFlightPlanInput(input, now = new Date()) {
  ...
  return {
    ...
    sid: normalizeUpper(input.sid || input.sidIdent || input.sid_ident),
    sidTransition: normalizeUpper(
      input.sidTransition || input.sidTrans || input.sid_trans || input.transition
    ),
...
function validateFlightPlan(plan, existingPlans) {
  ...
  for (const [field, code, airport] of [
    ...
    if (!code) {
      ...
      airportWarnings.push(`${code} nao encontrado na base local; aceito com revisao manual`);
    } else if (!airport.ifr) {
      airportWarnings.push(`${code} nao possui confirmacao IFR na base local`);
    } else if (aircraft && airport.runwayMeters > 0 && airport.runwayMeters < aircraft.minRunwayMeters) {
      airportIssues.push(`${code} possui pista curta para ${plan.aircraftType}`);
    } else if (aircraft && !airport.runwayMeters) {
      airportWarnings.push(`${code} sem pista cadastrada; compatibilidade exige revisao manual`);
    }
  }
  ...
  } else if (airportWarnings.length) {
    warnStep(steps, "airports", "Aeroportos", airportWarnings.join("; "));
  } else {
    passStep(
      steps,
      "airports",
  ...
  } else if (!/^[A-Z]{4}$/.test(plan.alternate)) {
    failStep(steps, "alternate", "Alternado", "Alternado com ICAO invalido.");
  } else if (!alternate) {
    warnStep(steps, "alternate", "Alternado", "Alternado aceito com revisao manual.");
  } else {
...
function buildClearance(plan) {
  const departureProcedure = [plan.sid, plan.sidTransition].filter(Boolean).join(" ");
  ...
  const destinationLine = departureProcedure
    ? `${plan.callsign} CLEARED TO ${plan.destination} VIA ${departureProcedure}.`
    : `${plan.callsign} CLEARED TO ${plan.destination}.`;
...
function publicFlightPlan(plan) {
  return {
    ...
    sid: plan.sid,
    sidTransition: plan.sidTransition,
    sidIdent: plan.sid,
    sidTrans: plan.sidTransition,
    sid_ident: plan.sid,
    sid_trans: plan.sidTransition,
    status: plan.status,
    squawk: plan.squawk,
